...
export async function buildProductPages({ mode, catalog }) {
    const productList = Object.entries(catalog)
    ...
    for (const [name, product] of productList) {
        if (!product.carousel)
            console.error("NO PRODUCT", name, product);
    }
    const pages = await Promise.all(productList.map(async ([pageName, product]) => {
        ...
        return {
            ...
            html: (product.relocated
                ...
                : await minify(await productPageHtml({
                    ...context,
                    mode,
                    catalog,
                    product,
    ...
    async function writePage(pageName, text) {
    ...
    await Promise.all(pages.map(async (page) => {
        ...
        for (const redirect of page.redirects) {
            ...
            const redirectPath = await writePage(redirect, `<meta http-equiv="refresh" content="0;URL='${destinationUrl}'" />`);
            console.log("write redirect", redirectPath);
        }
    }));
}
//# sourceMappingURL=build-product-pages.js.map