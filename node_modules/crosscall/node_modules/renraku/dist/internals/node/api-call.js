...
export async function apiCall({ id, debug, logger, expose, headers, body = "", }) {
    // parse and validate an incoming "order"
    const order = JSON.parse(body.trim());
    if (!order.topic)
        throw err(400, `request 'topic' string required`);
    if (!order.func)
        throw err(400, `request 'func' string required`);
    ...
    try {
        ...
        if (!topic)
            throw err(400, `unknown topic "${topic}"`);
        // fetch the method function
        const method = topic[order.func];
        if (!method)
            throw err(400, `unknown topic method "${order.topic}.${order.func}"`);
        ...
        if (debug)
            logger.debug("arguments:", order.params);
        if (debug)
