...
export async function apiServer({ expose, debug = false, logger = new DisabledLogger, }) {
    return {
        start(port) { }
    };
}
const noopAugmentation = async (request) => async (result) => ({ result });
export function simpleServerTopic(topic) {
    return serverizeTopic(noopAugmentation, topic);
}
export function simpleServerApi(api) {
