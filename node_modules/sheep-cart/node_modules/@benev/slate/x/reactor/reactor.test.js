...
function setup() {
    return new Reactor(new Flat(), new SignalTower());
}
export default {
    "two-function syntax": {
        "reactor reacts to flatstate changes": async () => {
            ...
            let calls = 0;
            reactor.reaction(() => void state.alpha, () => calls++);
            state.alpha++;
            await reactor.wait;
        ...
        "reactor reacts to signal changes": async () => {
            ...
            let calls = 0;
            reactor.reaction(() => void bravo.value, () => calls++);
            bravo.value++;
            await reactor.wait;
            expect(calls).equals(1);
        },
        ...
        "reaction can be stopped": async () => {
            ...
            const bravo = reactor.signals.signal(0);
            let calls = 0;
            const stop = reactor.reaction(() => void bravo.value, () => calls++);
            stop();
            state.alpha++;
            bravo.value++;
            await reactor.wait;
